#example-id.ipynp 
#NP for 1d functions
#how to train and sample from a Neural Process for a simple class of 1D functions.
#The functions are defined by f(x) = a * sin(x - b) where a and b and randomly sampled.
...
for i in range(64):
    ...
    plt.plot(x_target.numpy()[0], mu.detach().numpy()[0], 
             c='b', alpha=0.5)
    plt.xlim(-pi, pi)
plt.savefig('data_trained_prior.png')
plt.figure()
...
plt.savefig('data_posterior.png')